class TableManager {
  constructor() {
    // Inicializa variables y configura Firebase
    this.currentTableId = null;
    this.tables = []; // Almacena las tablas cargadas desde Firebase
    this.db = null; // Referencia a la base de datos de Firebase
    ...
    try {
      ...
      this.showNotification(
        "Error: No se pudo conectar a Firebase. Revisa las credenciales.",
        "error"
      );
    }
  ...
  async init() {
    // Configura los eventos y carga las tablas
  ...
  setupEventListeners() {
    ...
    // Close tables modal
    document
      .getElementById("closeTablesModal")
  ...
  insertSubcellsToEditableCells() {
    ...
    Array.from(container.children).forEach((child) => {
      ...
      if (
        ...
        if (
          child.classList.contains("experience-cell") ||
          child.classList.contains("mark-cell")
        )
          return;
        ...
        if (shouldExclude) {
          ...
          if (existing) {
            const firstText = (
              existing.querySelector(".subcell")?.textContent || ""
            ).trim();
            child.innerHTML = "";
  ...
  toggleExperienceCell(cell) {
    ...
    document
      .querySelectorAll(`[data-col="${col}"].experience-cell`)
      .forEach((c) => {
  ...
  async loadAllTablesFromFirebase() {
    ...
    try {
      ...
      this.showNotification(
        "Error de sincronización: No se pudieron cargar las tablas.",
        "error"
      );
    }
  }
  ...
  async updateTableNameInFirebase(tableId, newName) {
    ...
    try {
      ...
      this.showNotification(
        "❌ Error al renombrar la tabla en la nube.",
        "error"
      );
      return false;
    }
  ...
  async deleteTable(tableId) {
    ...
    if (
      !confirm(
        "¿Está seguro de que desea eliminar esta tabla de la NUBE? Esta acción es irreversible."
      )
    )
      return;
  ...
  loadTableData(data) {
    ...
    // Fallback: Check heuristics if no version is present
      ...
      isOldSchema = true;
      console.log(
        "Detectado esquema antiguo por cantidad de celdas (" +
          dataKeys.length +
          "). Migrando..."
      );
    }
    ...
    if (isOldSchema) {
      ...
      Object.entries(data.editableCells).forEach(([key, content]) => {
        ...
        if (oldIndex === 0) newIndex = 0; // Afiliación -> Afiliación (Antes)
          ...
          newIndex = 4; // Relacionamiento -> Relacionamiento (Durante)
    ...
    Object.entries(data.experienceCells || {}).forEach(([col, emoji]) => {
      ...
      const cell = document.querySelector(
        `[data-col="${col}"][data-emoji="${emoji}"].experience-cell`
      );
      if (cell) {
        cell.classList.add("selected");
  ...
  async handleNameSave() {
    ...
    if (this.nameModalAction === "new") {
      ...
      const success = await this.updateTableNameInFirebase(
        this.nameModalTableId,
        name
      );
      if (success) {
        ...
        (table) => `
            <div class="table-item">
                ...
                <div class="table-actions">
                    <button class="btn-primary btn-small" onclick="tableManager.loadTable('${
                      table.id
                    }'); tableManager.hideTablesModal();">
                        Cargar
                    </button>
                    <button class="btn-secondary btn-small" onclick="tableManager.showNameModal('rename', '${
                      table.id
                    }')">
                        Renombrar
                    </button>
                    <button class="btn-danger btn-small" onclick="tableManager.deleteTable('${
                      table.id
                    }')">
                        Eliminar
                    </button>
  ...
  exportToPDF() {
    ...
    const now = new Date();
    doc.text(
      `Generado: ${now.toLocaleDateString("es-ES")} ${now.toLocaleTimeString(
        "es-ES"
      )}`,
      pageWidth - margin - 50,
      20
  ...
  drawPhaseHeaders(
    ...
    doc.text(
      "ANTES",
      margin + labelColumnWidth + dataColumnWidth * 2 - 10,
      yPosition + 8
    );

    // Encabezado DURANTE (4 columnas) - mismo color de fondo
    doc.rect(
      margin + labelColumnWidth + dataColumnWidth * 4,
      yPosition,
      dataColumnWidth * 4,
      rowHeight,
      "FD"
    ...
    doc.text(
      "DURANTE",
      margin + labelColumnWidth + dataColumnWidth * 6 - 12,
      yPosition + 8
    );

    // Encabezado DESPUÉS (4 columnas) - mismo color de fondo
    doc.rect(
      margin + labelColumnWidth + dataColumnWidth * 8,
      yPosition,
      dataColumnWidth * 4,
      rowHeight,
      "FD"
    ...
    doc.text(
      "DESPUÉS",
      margin + labelColumnWidth + dataColumnWidth * 10 - 12,
      yPosition + 8
    );
  }
